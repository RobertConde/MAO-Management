...
$sql_conn = getDBConn();
if (!is_a($payment_stmt = $sql_conn->query("SELECT c.competition_id, c.competition_description, ca.unique_id FROM competitions c LEFT OUTER JOIN competition_approvals ca ON c.payment_id = ca.id AND ca.id = $id ORDER BY ISNULL(ca.unique_id), ca.unique_id, ca.competition_id;"), 'mysqli_result'))
