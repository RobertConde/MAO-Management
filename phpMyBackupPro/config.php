...
if (!isset($CONF['no_login'])) $CONF['no_login'] = '';
...
if (isset($_POST['submit'])) {
	...
	if (PMBP_save_global_conf()) {
		$out = PMBP_addOutput(C_SAVED . "!", "green");
	} else {
		$out = PMBP_addOutput(PMBP_GLOBAL_CONF . " " . C_WRITEABLE . "!", "red");
	}
}
...
foreach ($CONF as $key => $value) {
	if (array_key_exists($key, $validate)) {
		switch ($validate[$key]) {
			case "int":
				if (!preg_match("/^[0-9]*$/i", $value)) echo PMBP_addOutput(array_search($key, $conf) . "' " . C_WRONG_TYPE, "red");
				break;
			case "float":
				if (!preg_match("/^[0-9]*\.?[0-9]*$/i", $value)) echo PMBP_addOutput(array_search($key, $conf) . "' " . C_WRONG_TYPE, "red");
				break;
			case "email":
				if ($value || $CONF['email_use']) {
					foreach (explode(",", $value) as $value2) {
						if (!preg_match("/^\ *[������a-zA-Z0-9_-]+(\.[������a-zA-Z0-9\._-]+)*@([������a-zA-Z0-9-]+\.)+([a-z]{2,4})$/i", $value2)) {
							echo PMBP_addOutput(array_search($key, $conf) . "' " . C_WRONG_TYPE, "red");
							break;
						}
					}
				}
...
if (isset($_SESSION['sql_host_org'])) {
	if (!@mysql_connect($_SESSION['sql_host_org'], $_SESSION['sql_user_org'], $_SESSION['sql_passwd_org'])) echo PMBP_addOutput(C_WRONG_SQL, "red");
	if ($_SESSION['sql_db_org']) if (!@mysql_select_db($_SESSION['sql_db_org'])) echo PMBP_addOutput(C_WRONG_DB, "red");
} else {
	if (!PMBP_mysql_connect()) echo PMBP_addOutput(C_WRONG_SQL, "red");
	if ($CONF['sql_db']) if (!@mysql_select_db($CONF['sql_db'])) echo PMBP_addOutput(C_WRONG_DB, "red");
}
...
if ($_SESSION['multi_user_mode']) {
	...
	foreach ($PMBP_SYS_VAR as $key => $value) {
		if ($i % 2 == 0) echo "<tr>\n";
		echo "<td colspan=\"2\">" . $key . ":</td><td colspan=\"2\"><input type=\"text\" size=\"50\" name=\"" . $key . "\" value=\"" . $value . "\"></td>\n";
		if ($i % 2 == 1) echo "</tr>";
		$i++;
	}
	if ($i % 2) echo "</tr>\n";
...
function PMBP_config_print($item)
{
	...
	$confItem = htmlspecialchars($CONF[$item]);
	if (isset($checkbox[$item])) {
		if ($confItem) $status = " checked"; else $status = " ";
		$out = "<input type=\"checkbox\" name=\"" . $item . "\" value=\"" . $confItem . "\"" . $status . PMBP_config_disable($item) . ">";
	} elseif (in_array($item, $password)) {
		$out = "<input type=\"password\" size=\"50\" name=\"" . $item . "\" value=\"" . $confItem . "\"" . PMBP_config_disable($item) . ">";
	} elseif (isset($select[$item])) {
		$out = "<select name=\"" . $item . "\"" . PMBP_config_disable($item) . ">\n";
		foreach ($select[$item] as $opt_no => $print_value)
			if ($opt_no == $confItem) $out .= "<option value=\"" . $opt_no . "\" selected>" . $print_value . "</option>\n";
		...
		foreach ($files as $file)
			if ($file == $confItem) $out .= "<option value=\"" . $file . "\" selected>" . $file . "</option>\n"; else $out .= "<option value=\"" . $file . "\">" . $file . "</option>\n";
		$out .= "</select>";
	} else {
		$out = "<input type=\"text\" size=\"50\" name=\"" . $item . "\" value=\"" . $confItem . "\"" . PMBP_config_disable($item) . "><br>";
	}
	return $out;
}
