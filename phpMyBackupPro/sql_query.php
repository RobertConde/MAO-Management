...
if (!$con = PMBP_mysql_connect()) echo PMBP_addOutput(I_SQL_ERROR, "red_left") . "<br>";
...
if (($_POST['sql_query'] || $_FILES['sql_file']) && $_POST['db']) {
			...
			if (is_array($all_queries)) {
				foreach ($all_queries as $query) {
					...
					if ((!$res = mysql_query($query) and mysql_errno() != 1065) or (mysql_errno() == 1065 and !$sql_error)) {
						echo $query . "\n" . PMBP_addOutput(mysql_error(), "red_left") . "<br>\n";
	...
	if (!isset($_POST['fragmented'])) {
		...
		if ($sql_error) echo PMBP_addOutput(SQ_ERROR . " " . $sql_error, "red_left");
		else echo PMBP_addOutput(SQ_SUCCESS, "green_left");
