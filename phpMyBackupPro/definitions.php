...
// definitions
define('PMBP_VERSION', "2.5");  // this is the version of this phpMyBackupPro release
...
if (!include_once($prepath . PMBP_GLOBAL_CONF)) {
	// show warning in scheduled mode only if scheduled_debug is on
...
// LOGGED_IN initialisieren
if (!isset($_SESSION['LOGGED_IN'])) $_SESSION['LOGGED_IN'] = false;

// set working sql server
if (count($CONF['sql_host_s'])) {
	// set working server and register session vars
	if (!isset($_SESSION['sql_host_org']) || !$_SESSION['LOGGED_IN']) $_SESSION['sql_host_org'] = $CONF['sql_host'];
	if (!isset($_SESSION['sql_user_org']) || !$_SESSION['LOGGED_IN']) $_SESSION['sql_user_org'] = $CONF['sql_user'];
	if (!isset($_SESSION['sql_passwd_org']) || !$_SESSION['LOGGED_IN']) $_SESSION['sql_passwd_org'] = $CONF['sql_passwd'];
	if (!isset($_SESSION['sql_db_org']) || !$_SESSION['LOGGED_IN']) $_SESSION['sql_db_org'] = $CONF['sql_db'];
	if (!isset($_SESSION['wss']) || !$_SESSION['LOGGED_IN']) $_SESSION['wss'] = -1;
...
if (!$_SESSION['multi_user_mode']) {
	...
	if (count($CONF['sql_host_s']) && basename($_SERVER['SCRIPT_NAME']) !== "config.php") {
		...
		if ($_SESSION['wss'] < 0) {
			...
			// other accounts
			define('PMBP_EXPORT_DIR', $prepath . $_PMBP_EXPORT_DIR . $CONF['sql_host'] . "_" . $CONF['sql_user'] . "/");
		}
	} else {
		// single db mode
		define('PMBP_EXPORT_DIR', $prepath . $_PMBP_EXPORT_DIR);
	...
	// multi user mode
	define('PMBP_EXPORT_DIR', $prepath . $PMBP_MU_CONF['user_export_dir']);
}
...
if (isset($_POST['lang']) && preg_replace("#.*/#", "", $_SERVER['PHP_SELF']) == "config.php") {
	if (preg_match("#^[a-zA-Z ]*$#", $_POST['lang'])) {
